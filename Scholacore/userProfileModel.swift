...
//  Scholacore
//
//  Created by Tarun kaushik on 01/05/17.
...
class userProfileModel:NSObject {
    let storageRef = Constants.storageReference.userProfileImage
    let dbRef = Constants.dataBaseReference.userProfile
    var user = User()
    ...
    func loadFromFirebaseDb(){
        ...
        dbRef.child(userId!).observeSingleEvent(of: .value, with: { (snapShot) in
            if let userSnap = snapShot.value as? NSDictionary{
                ...
                self.saveUserData(userName: self.username  , userRoll: self.userRoll, userCourse: self.userCourse)
